...
class Model:
    ...
    def train(
        ...
        if self.skip_test_loss:
            self.train_state.set_data_test(None, None, None)
        else:
    ...
    def _test(self):
        ...
        if self.skip_test_loss:
            self.train_state.y_pred_test = None
            self.train_state.loss_test = np.asarray([], dtype=float)
        ...
        if np.isnan(self.train_state.loss_train).any():
...
class TrainState:
    ...
    def update_best(self):
        if self.best_loss_train > np.sum(self.loss_train):
            ...
            self.best_loss_test = (
                np.nan if self.loss_test is None or np.size(self.loss_test) == 0 else np.sum(self.loss_test)
            )
