...
class Rectangle(Hypercube):
    '''Hypercube 是用于定义高维超立方体，Rectangle 是 Hypercube 的子类，Rectangle 专门针对二维空间中的矩形进行了扩展和定制
    继承自 Hypercube的核心方法：内部点采样 uniform_points()可选择是否包含边界点,random_points()
                            判断点x是否在矩形内部，是否在矩形边界上
    子类 Rectangle 覆盖/重写的方法：uniform_boundary_points(n)在矩形边界生成均匀分布的边界点
                            random_boundary_points(n)在矩形边界生成随机分布的边界点
    ...
    def __init__(self, xmin, xmax):
        ...
        self.perimeter = 2 * np.sum(self.xmax - self.xmin)#计算周长
        self.area = np.prod(self.xmax - self.xmin)#计算面积

    def uniform_boundary_points(self, n):
        nx, ny = np.ceil(n / self.perimeter * (self.xmax - self.xmin)).astype(int)
        # 分四边生成点，每条边的点数按其长度占总周长的比例分配
        xbot = np.hstack(
            (
    ...
    def random_boundary_points(self, n, random="pseudo"):
        l1 = self.xmax[0] - self.xmin[0]# 底边长度
        l2 = l1 + self.xmax[1] - self.xmin[1]# 底边 + 右边总长
        l3 = l2 + l1 # 底边 + 右边 + 顶边总长
        u = np.ravel(sample(n + 2, 1, random))# 生成 n+2 个随机数
        # Remove the possible points very close to the corners
        u = u[np.logical_not(np.isclose(u, l1 / self.perimeter))]# 移除底边右端点
        u = u[np.logical_not(np.isclose(u, l3 / self.perimeter))]# 移除顶边左端点
        u = u[:n]# 移除顶边左端点

        u *= self.perimeter# 将归一化长度转换为实际坐标
        x = []
        # 映射到具体边
        for l in u:
            if l < l1:
