...
class TrainingDisplay:
    ...
    def __call__(self, train_state):
        if not self.is_header_print:
            ...
            self.len_test = max(len(train_state.loss_test) * 10 + 4, len("Test loss") + 2)
            self.len_metric = max(len(train_state.metrics_test) * 10 + 4, len("Test metric") + 2)
            self.header()
        self.print_one(
    ...
    def summary(self, train_state):
        ...
        if train_state.best_loss_test is None or np.isnan(train_state.best_loss_test):
            print("  test loss: not computed")
        else:
