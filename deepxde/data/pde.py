...
class PDE(Data):
    ...
    def losses(self, targets, outputs, loss_fn, inputs, model, aux=None):
        ...
        if not isinstance(loss_fn, (list, tuple)):
            ...
            raise ValueError(
                "There are {} PDE/BC errors, but only {} loss functions.".format(
        ...
        for i, bc in enumerate(self.bcs):
            ...
            # 修复：使用正确的 loss_fn 和参数
            losses.append(loss_fn[len(f) + i](bkd.zeros_like(error), error))

        # Add auxiliary losses (frequency regularization loss functions)
        if hasattr(model, "auxiliary_losses") and model.auxiliary_losses:
            for aux_loss_fn in model.auxiliary_losses:
                # Call the auxiliary loss function to get the loss tensor
                loss_tensor = aux_loss_fn()
    ...
    def test_points(self):
        '''​TODO 意图：希望测试边界点独立生成（与训练点不同），但当前代码未实现这一点。'''
